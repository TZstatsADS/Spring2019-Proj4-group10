...
svm.pred <- predict(svm.model.3, train_mat[,-1])
accuracy<- mean(svm.pred==train_mat$ifwordcorrect)
save(svm.model, file = "../output/svm.model.best.rda")
prederror <- as.data.frame(datamatrix)
prederror$predifwordcorrect<-svm.pred
save(prederror, file = '../output/prederror.RData')
```
