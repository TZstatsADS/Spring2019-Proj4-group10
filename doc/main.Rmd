...
Here we implement Method / Paper 3 using a SVM model trained on the 12 features outlined in the paper. We first split the dataset into training and testing datasets (80% and 20% respectively). From this point on, we train the SVM model and use the trained model to predict whether the word is correct or not (error detection) on the test dataset. From this point on, only the test dataset is used for error detection, error correction, and final evaluation metrics for objective scoring purposes. 
```{r}
#see the svm.R
...
Here, we implement the first paper using digram features to correct errors for the words that the SVM model predicted to contain errors. This paper only outlines the error correction algorithm for the case in which only 1 error exists. However, we have extended the algorithm ourselves to cover the case in which 2 errors exist within the word.
```{r}
#see correction.R
...
Here, we implemented both word-level and character-level evaluation metrics (recall and precision). For the character-level evaluation, since the order of the characters does not matter (according to Professor Liu's Piazza post), we have counted the number of each alphabets in the ground truth text, tesseract text, and corrected text (using error detection + error correction), and used those features to calculate recall and precision.
```{r, warning=FALSE}
