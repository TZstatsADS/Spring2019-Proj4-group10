...
```{r}
demo <- unlist(strsplit(tolower(readLines(paste("../data/ground_truth/",file_name_vec[1],sep=""), warn=FALSE)), " "))
cbind(tes_split_list[1:30], datamatrix[1:30,'ifwordcorrect'], demo[1:30])
```

```{r}
tttest <- cbind(test_vec, test_if_error, aftercorrection_vec, (tolower(test_vec)==tolower(aftercorrection_vec)))
tttest[1:100,]
```
